...
class GetModelSuitesResponse(BaseModel):
    """
    GetModelSuitesResponse
    """  # noqa: E501

    model_suites: List[LLMSuiteConfigWithGenerationParams]
    __properties: ClassVar[List[str]] = ["model_suites"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in model_suites (list)
        _items = []
        if self.model_suites:
            for _item in self.model_suites:
                if _item:
                    _items.append(_item.to_dict())
            _dict["model_suites"] = _items
        return _dict
