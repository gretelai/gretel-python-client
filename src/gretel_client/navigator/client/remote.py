...
class RemoteClient(ClientAdapter[Serializable]):
    ...
    def __init__(
        self,
        jarvis_endpoint: str = "https://jarvis.dev.gretel.cloud",
    ):
        ...
        logger.debug(f"🌎 Connecting to {self._jarvis_endpoint}")

    # todo: pass an event handler and log non task outputs
    def run_task(
        self, name: str, config: dict, inputs: list[TaskInput], verbose: bool = False
    ) -> TaskOutput:
        if config is None:
            config = {}
        if inputs is None:
        ...
        response = requests.post(
            ...
            json={"name": name, "config": config, "inputs": inputs},
    ...
    def submit_batch_workflow(
        ...
        print(f"🔗 -> {project.get_console_url()}")
    ...
    def get_step_output(
        ...
        with self._request_artifact(workflow_run_id, step_name, format) as response:
            ...
            if content_type == "application/json":
                ...
                raise Exception(
                    f"Cannot get output format {format!r} as TaskOuput. Try downloading instead."
                )
    ...
    def _request_artifact(
        ...
        endpoint = f"{self._jarvis_endpoint}/workflows/{workflow_run_id}/{step_name}"
