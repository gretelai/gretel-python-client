...
DATA_DESIGNER_NL_SYSTEM_PROMPT = DATA_DESIGNER_BASE_SYSTEM_PROMPT.format(
    llm_type_specific_instructions="""\
You are particularly adept at writing natural language, strictly adhering to the \
to all formatting constraints and instructions provided to you.
"""
)
...
Your task is to generate the `{name}` column in a dataset based on the information given below. \
...
### General Instructions ###
    * Generate the `{name}` column as described above.
    * Remember to generate all output in English.
    * Remember to base your response on the above information. \
    {extra_instructions}
    * {parser_instructions}
...
system_prompt_dict = {
    "nl": DATA_DESIGNER_NL_SYSTEM_PROMPT,
