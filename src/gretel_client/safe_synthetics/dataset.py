    ...
    def synthesize(
            ...
            if isinstance(config, dict):
                ...
                try:
                    ...
                    raise TaskConfigError(
                        f"{model_or_blueprint_or_task!r} is not a "
                        f"valid model identifier. Must be one of "
                        f"{', '.join(self._synthetic_model_registry.models)}"
                    )
                task_config = config
        ...
        if num_records:
            try:
                ...
                raise TaskConfigError(
                    "We tried configure num_records, but "
...
class SafeSyntheticDatasetFactory:
    ...
    def from_data_source(
        self,
        data_source,
        holdout: Optional[Union[float, int]] = 0.5,
    ...
    def workflow_id_callback(self, workflow_id: str):
