...
class FakeConstant(Transformer):
    ...
    def __init__(self, config: FakeConstantConfig):
        ...
        self.faker = Fakers(seed=config.seed, locales=config.locales, locale_seed=config.locale_seed)
        self.fake_method = config.fake_method
        if self.labels:
            if not all(elem in VALID_LABELS for elem in self.labels):
                raise ValueError(f"Labels list has to be one or more of {VALID_LABELS} only!")
        elif not self.fake_method:
            raise ValueError("No fake methods or labels which map to fake methods are specified!")
