...
class WorkflowBuilder:
    ...
    def builder(
        ...
        """Create a new instance of WorkflowBuilder.

        Returns:
            Self: A new WorkflowBuilder instance.
        """
        return cls(
            project_id, api_provider, resource_provider, workflow_session_manager
        )
            ...
            try:
                for output in response.iter_lines():
                    try:
                        ...
                        logger.error(f"Could not deserialize message: {output}")
