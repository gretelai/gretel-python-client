...
from gretel_client.workflows.configs.workflows import Workflow as WorkflowConfig
...
class WorkflowRun:
    ...
    def get_step_output(
        ...
        with self._api_provider.requests().get(endpoint, stream=True) as response:
            ...
            if output_type in ("evaluate_ss_dataset", "evaluate_dataset"):
                return Report.from_bytes(response_bytes)
    ...
    def workflow(self) -> WorkflowConfig:
        return WorkflowConfig(**self._api_response.config or {})
